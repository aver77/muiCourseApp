...
const WithIntl = ({children}) => {
    const [localization, setLocalization] = useState(getLocalStorageItem(SWITCH_LANGUAGES) ?? false)
    ...
    return (
        <IntlProvider
            messages={languages[ternaryOperation(!localization, LOCALES.RUSSIAN, LOCALES.ENGLISH)]}
            locale={ternaryOperation(!localization, LOCALES.RUSSIAN, LOCALES.ENGLISH)}
